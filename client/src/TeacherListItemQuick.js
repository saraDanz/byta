...
import { Checkbox, Paper, Table, TableContainer, TableRow, TableCell, TableHead, TableBody } from "@mui/material"
// import ListItemIcon from '@mui/material/ListItemIcon';
// import ListItemText from '@mui/material/ListItemText';
...
import { updateCourseStatus } from './services/teacherCourseService';
export default function TeacherListItemQuick({  item, updateStatus, deleteTeacher, editFare, editTeacher, deleteTeacherFromCourse, withReports }) {
    const [open, setOpen] = useState(false);
    const [isUpdatePending, setIsUpdatePending] = useState(false);
    const updateTecherCourseStatus = (courseId, status) => {
        setIsUpdatePending(true);
        updateCourseStatus(courseId,item._id, status).then(res => {
            updateStatus(item._id, courseId, status)
            alert("סטטוס מורה בקורס עודכן בהצלחה ")
        }).catch(err => {
            console.log(err)
            alert("לא ניתן לעדכן את הסטטוס של קורס זה " + err.message)
        }).finally(()=>{
                            ...
                            <TableRow key={row.name}>
                                ...
                                <TableCell style={{ width: 160 }} align="right">
                                  ...
                                  disabled={isUpdatePending}
                                        checked={row.isActive == undefined ? true : row.isActive}
                                        onChange={(e) => { updateTecherCourseStatus(row._id, e.target.checked) }}
