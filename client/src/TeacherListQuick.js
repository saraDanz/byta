...
export default function TeacherListQuick() {
    ...
    const getTotalPages = () => {
        setCurrentPage(-1)
        axios.get(BASE_URL + "users/totalTeacherPages?s=" + (searchField.current.value || "")).then(res => {
    ...
    useEffect(() => {
        ...
        if (currentPage == -1)
            return;
        axios.get(BASE_URL + "teacherCourses/withTheirCoursesLimit?page=" + currentPage + "&s=" + (searchField.current.value || "")).
            then(res => {
                console.log(res.data);
                let t = res.data;
                setTeachers(t)
