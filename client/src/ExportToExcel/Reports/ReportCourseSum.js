    ...
    for (let teacherTz in a) {
        ...
        for (let courseId in a[teacherTz])
            courses.push({
                ...
                totalDays: sumByField('numDays', a[teacherTz][courseId]),//.......group by date
                totalFrontalDays: a[teacherTz][courseId].filter(u => u.type == "frontal").length,///.......group by date
    ...
    const exportToExcel = () => {
        ...
        if (b.length) {
            let rep = b.map(item => {
                ...
                let workerNum = item.courses[0].reports[0].teacherName;
                let totalFrontalDays = item.totalFrontalDays;
                let totalDays = item.totalDays;
                let totalHours = item.totalNumHours;
                return {
                    teacherName, tz, workerNum, totalHours, totalDays, totalFrontalDays
                }
            })

            //לסנן עמודות לא רלוונטיות ולשלוח כפרמטר שמות לעמודות
            exportToCSV(rep, fromDate && toDate ? `סכום שעות למורה לתאריכים ${fromDate.$d.toLocaleDateString()}-${toDate.$d.toLocaleDateString()}: ` : `סכום שעות למורה -${year}-${month}`, [['שם מורה', "תז", "מספר עובד", "מספר שעות", "מספר ימים", "מספר ימים פרונטליים"]])
        }
    }
